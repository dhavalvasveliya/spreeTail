...
def select_month(driver,monthYear,dateinput): #e.g monthYear=December 2021 #dateinput=either start or end
    while True:
        ...
        if check==target:
            ...
            if dateinput=='start':
                driver.find_element(By.CLASS_NAME,'rdtPrev').click()
            else:
                driver.find_elements(By.CLASS_NAME,'rdtPrev')[1].click()
...
def get_data(driver,site):
    date_today = datetime.datetime(2021,7,23)
    for single_date in (date_today + timedelta(n) for n in range(21)):
        date_yesterday = single_date-datetime.timedelta(days=1)
